...
class PersonEditCtrl {
    ...
    public function validateSave() {
        ...
        if (empty(trim($this->form->login))) {
            Utils::addErrorMessage('Wprowadź swój unikalny login');
        }
        if (empty(trim($this->form->password))) {
            Utils::addErrorMessage('Wprowadź swóje hasło dostępowe');
        }
    ...
    public function action_personSave() {
        ...
        if ($this->validateSave()) {
            ...
            try {
                ...
                Utils::addErrorMessage('Wystąpił nieoczekiwany błąd podczas zapisu użytkownika');
                if (App::getConf()->debug)
                    Utils::addErrorMessage($e->getMessage());
            }
